...
export default async function updateCommands(client) {
    ...
    try {
        ...
        const data = await rest.put(Routes.applicationCommands(process.env.discord_application_id), { body: restCommands });
        const dataRestricted = await rest.put(Routes.applicationGuildCommands(process.env.discord_application_id, process.env.discord_admin_guild), { body: restCommandsForAdminGuild });
